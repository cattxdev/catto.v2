...
type WizardAction =
  | { type: 'SELECT_TYPE'; selectedType: UploadableType }
  | { type: 'PREV_STEP' }
  | { type: 'NEXT_STEP' }
...
function wizardReducer(state: WizardState, action: WizardAction): WizardState {
  switch (action.type) {
    ...
    case 'NEXT_STEP':
      return state.step < 3 ? { ...state, step: (state.step + 1) as WizardStep } : state;
    case 'ADD_FILES':
      return {
        ...state,
        files: [...state.files, ...action.files.map((f) => ({ file: f, description: '' }))],
      };
    case 'REMOVE_FILE':
    ...
    case 'FAB_UPLOAD':
      return { ...state, selectedType: 'IMAGE', step: 2 };
...
export function EvidenceWizard({ guildId, caseNumber, onUploadComplete }: EvidenceWizardProps) {
  ...
  const handleFileSelect = useCallback((selected: FileList | File[]) => {
    dispatch({ type: 'ADD_FILES', files: Array.from(selected) });
  }, []);
  ...
  const handleUpload = async () => {
    ...
    for (let i = 0; i < state.files.length; i++) {
      ...
      try {
        dispatch({ type: 'UPDATE_PROGRESS', index: i, patch: { status: 'uploading', progress: 10 } });
        ...
        dispatch({ type: 'UPDATE_PROGRESS', index: i, patch: { progress: 30 } });
        ...
        dispatch({ type: 'UPDATE_PROGRESS', index: i, patch: { status: 'hashing', progress: 60 } });

        const contentHash = await computeSHA256(entry.file);
        dispatch({ type: 'UPDATE_PROGRESS', index: i, patch: { status: 'confirming', progress: 80 } });
      ...
      <div className="p-5">
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-2 sm:grid-cols-3">
              {TYPE_OPTIONS.map((opt) => {
                ...
                const isSelected = state.selectedType === opt.type;
                return (
                  <button
                    key={opt.type}
                    onClick={() => dispatch({ type: 'SELECT_TYPE', selectedType: opt.type })}
        ...
        {state.step === 2 && fileMode && (
          <div>
            ...
            {/* Drop zone */}
            <div
              onDragOver={(e) => { e.preventDefault(); dispatch({ type: 'SET_DRAG_OVER', dragOver: true }); }}
            ...
            >
              ...
              <input
                ...
                type="file"
                multiple
                accept={MIME_FILTERS[state.selectedType!] ?? '*/*'}
                className="hidden"
                onChange={(e) => {
              ...
              <div className="mt-3 space-y-2">
                {state.files.map((entry, i) => (
                  ...
                  >
                    ...
                    <button onClick={() => dispatch({ type: 'REMOVE_FILE', index: i })} className="shrink-0 text-[var(--mod-text-dim)] hover:text-[var(--mono-white)]">
                      <IconX size={14} />
                    </button>
                  </div>
                ))}
        ...
        {state.step === 2 && urlMode && (
          <div>
            ...
            <input
              ...
              placeholder={state.selectedType === 'DISCORD_URL' ? 'https://discord.com/channels/...' : 'https://...'}
        ...
        {state.step === 3 && (
          <div className="space-y-4">
            ...
            <div>
              ...
              <TagSelector value={state.tags} onChange={(tags) => dispatch({ type: 'SET_TAGS', tags })} />
            </div>
            ...
            {fileMode && state.files.length > 1 && (
              <div>
                <div className="mb-2 flex items-center gap-2">
                  ...
                  >
                    {state.applyDescToAll && <IconCheck size={12} className="text-[var(--mono-white)]" />}
                  </button>
                    ...
                    {state.files.map((entry, i) => (
                      <div key={i}>
                        ...
                        <input
                          ...
                          onChange={(e) => dispatch({ type: 'UPDATE_FILE_DESC', index: i, description: e.target.value })}
              ...
              <div className="space-y-2">
                {state.uploadProgress.map((p) => (
                  <div key={p.index} className="flex items-center gap-2 text-xs">
                    <span className="min-w-0 flex-1 truncate text-[var(--mono-white)]">{state.files[p.index]?.file.name}</span>
