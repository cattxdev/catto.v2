...
export class ReputationService {
  ...
  async validateVouch(
    ...
    if (giver.id === receiver.id) {
      return {
        isValid: false,
        reason: `${EMOJIS.STATUS.ERROR} You cannot vouch for yourself.`,
      };
    }
    ...
    if (receiver.user.bot) {
      return {
        isValid: false,
        reason: `${EMOJIS.STATUS.ERROR} You cannot vouch for bots.`,
      };
    }
    ...
    if (giverAccountAge < minAccountAge) {
      return {
        isValid: false,
        reason: `${EMOJIS.STATUS.ERROR} Your account must be at least ${VOUCH_CONFIG.MIN_ACCOUNT_AGE_DAYS} days old to vouch for others.`,
      };
    }
    ...
    if (giver.joinedAt) {
      ...
      if (serverAge < minServerAge) {
        return {
          isValid: false,
          reason: `${EMOJIS.STATUS.ERROR} You must be in this server for at least ${VOUCH_CONFIG.MIN_SERVER_AGE_DAYS} days to vouch for others.`,
        };
      }
    }
    ...
    if (giverRep.lastVouchGiven) {
      ...
      if (timeSinceLastVouch < cooldown) {
        ...
        return {
          isValid: false,
          reason: `${EMOJIS.STATUS.LOADING} You can vouch again in ${hours} hour(s).`,
    ...
    if (recentVouch) {
      ...
      return {
        isValid: false,
        reason: `${EMOJIS.STATUS.LOADING} You already vouched ${receiver.user.username} as ${vouchType}. You can vouch them again in ${daysRemaining} day(s).`,
      };
    }
