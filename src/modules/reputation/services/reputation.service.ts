...
export class ReputationService {
	...
	async validateVouch(
		...
		if (receiver.user.bot) {
			return {
				isValid: false,
				reason: '❌ You cannot vouch for bots.',
			};
		}
		...
		if (giverAccountAge < minAccountAge) {
			return {
				isValid: false,
				reason: `❌ Your account must be at least ${VOUCH_CONFIG.MIN_ACCOUNT_AGE_DAYS} days old to vouch for others.`,
			};
		}
		...
		if (giver.joinedAt) {
			...
			if (serverAge < minServerAge) {
				return {
					isValid: false,
					reason: `❌ You must be in this server for at least ${VOUCH_CONFIG.MIN_SERVER_AGE_DAYS} days to vouch for others.`,
				};
			}
		}
		...
		if (giverRep.lastVouchGiven) {
			...
			if (timeSinceLastVouch < cooldown) {
				...
				return {
					isValid: false,
					reason: `⏳ You can vouch again in ${hours} hour(s).`,
		...
		if (recentVouch) {
			...
			return {
				isValid: false,
				reason: `⏳ You already vouched ${receiver.user.username} as ${vouchType}. You can vouch them again in ${daysRemaining} day(s).`,
			};
		}
	...
	async getVouchHistory(guildId: string, userId: string, type: 'received' | 'given' = 'received') {
		const where = type === 'received' 
			? { guildId, receiverUserId: userId }
			: { guildId, giverUserId: userId };
