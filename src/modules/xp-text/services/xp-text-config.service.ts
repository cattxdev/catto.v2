 ...
 */
export async function getConfig(guildId: string, bypassCache: boolean = false): Promise<GuildXPConfig> {
	// Check cache first
	if (!bypassCache) {
 ...
 */
export async function updateConfig(guildId: string, data: UpdateXPConfigDTO): Promise<GuildXPConfig> {
 ...
 */
export async function getAllConfigs(limit: number = 100, offset: number = 0): Promise<GuildXPConfig[]> {
