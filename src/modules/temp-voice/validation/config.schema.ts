...
export const tempVoiceConfigSchema = z.object({
	...
	joinChannelIds: z.array(z.string().regex(/^\d{17,19}$/)).min(1, 'At least one join channel is required'),
	...
	defaultCategoryId: z.string().regex(/^\d{17,19}$/).nullable().optional(),
	...
	logChannelId: z.string().regex(/^\d{17,19}$/).nullable().optional(),
});
