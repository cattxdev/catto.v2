...
export async function resetUserVoiceXP(guildId: string, userId: string, reason?: string): Promise<void> {
	const userXP = await getUserVoiceXP(guildId, userId);
	if (!userXP) return;
...
export async function updateUserVoiceLevel(guildId: string, userId: string, newLevel: number): Promise<void> {
