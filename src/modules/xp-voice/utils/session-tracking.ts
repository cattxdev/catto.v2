 ...
 */
export async function getActiveSession(guildId: string, userId: string): Promise<ActiveSession | null> {
	...
	try {
		...
		container.logger.error(`[Voice XP] Failed to parse session data for ${guildId}:${userId}`, error);
		return null;
	}
}
 ...
 */
export async function updateSession(guildId: string, userId: string, updates: Partial<ActiveSession>): Promise<boolean> {
	const session = await getActiveSession(guildId, userId);
