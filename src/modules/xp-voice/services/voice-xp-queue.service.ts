...
class VoiceXPQueueService {
	...
	constructor() {
		...
		this.worker.on('completed', (job) => {
			container.logger.debug(`[Voice XP Queue] Job ${job.id} completed for guild ${job.data.guildId}`);
		});
	...
	public async scheduleGuildAwards(guildId: string): Promise<void> {
		try {
			...
			container.logger.error(`[Voice XP Queue] Error scheduling awards for guild ${guildId}:`, error);
			throw error;
		}
	}
	...
	public async unscheduleGuildAwards(guildId: string): Promise<void> {
		try {
			...
			container.logger.info(`[Voice XP Queue] Unscheduled per-minute XP awards for guild ${guildId}`);
		} catch (error) {
			container.logger.error(`[Voice XP Queue] Error unscheduling awards for guild ${guildId}:`, error);
		}
	}
