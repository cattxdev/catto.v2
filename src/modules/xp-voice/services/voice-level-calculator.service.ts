...
export function calculateVoiceLevel(config: GuildVoiceXPConfig, currentXP: number): VoiceLevelCalculation {
...
function calculateLevelFromTable(
	...
	const nextLevelXP = level < thresholds.length ? (thresholds[level] ?? currentLevelXP + 1000) : currentLevelXP + 1000;
	const xpInCurrentLevel = currentXP - currentLevelXP;
	const xpNeededForLevel = nextLevelXP - currentLevelXP;
	const progress = xpNeededForLevel > 0 ? (xpInCurrentLevel / xpNeededForLevel) * 100 : 0;
...
export function recalculateAllVoiceLevels(config: GuildVoiceXPConfig, users: Array<{ userId: string; xp: number }>): Array<{ userId: string; newLevel: number }> {
