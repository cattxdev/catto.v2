...
export class RedisCommand extends Command {
	...
	public override async chatInputRun(interaction: ChatInputCommandInteraction) {
		...
		try {
			...
			const connectedClients = lines.find(l => l.startsWith('connected_clients:'))?.split(':')[1] || '0';
			const usedMemory = lines.find(l => l.startsWith('used_memory_human:'))?.split(':')[1] || 'Unknown';
			const totalKeys = lines.find(l => l.startsWith('db0:'))?.split('keys=')[1]?.split(',')[0] || '0';
			...
			const errorEmbed = new EmbedBuilder()
				...
				.setDescription(`Failed to connect to Redis: ${error instanceof Error ? error.message : 'Unknown error'}`)
				.setColor(0xff0000)
				.setTimestamp();
