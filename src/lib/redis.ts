 ...
 */
export async function setCache(key: string, value: string | number | object, expirationSeconds?: number): Promise<void> {
 ...
 */
export async function setCooldown(userId: string, commandName: string, seconds: number): Promise<void> {
 ...
 */
export async function getTopFromSortedSet(key: string, count: number): Promise<Array<{ member: string; score: number }>> {
