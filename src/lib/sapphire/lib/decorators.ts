        ...
        const ctr = new ctor(context, {
          ...
          name:
            container.client.options.subcommandsAdvanced?.nameCommandsAutogenerated === true
              ? `${parentCommandName}/${typeof slashSubcommandParsed === 'object' ? slashSubcommandParsed.name : baseOptions.name}`
              : baseOptions.name,
    ...
    createProxy(target, {
      construct: (ctor, [context, baseOptions]: [Piece.Context, Piece.Options]) => {
        ...
        const ctr = new ctor(context, {
          ...
          name:
            container.client.options.subcommandsAdvanced?.nameCommandsAutogenerated === true
              ? `${parentCommandName}/${groupName}/${typeof slashSubcommandParsed === 'object' ? slashSubcommandParsed.name : baseOptions.name}`
              : baseOptions.name,
        }) as unknown as Command;

        return analizeSubcommandGroupParsed(
          ctr,
          parentCommandName,
          groupName,
          slashSubcommandParsed
        );
      },
    })
  );
