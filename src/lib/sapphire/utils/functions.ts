...
export const isCommandOptionsUpdated = (
  ...
  c2: SlashCommandSubcommandBuilder
...
export const parseSlashSubcommand = (
  ...
  subcommand
    ? typeof subcommand === 'function'
      ? subcommand(new SlashCommandSubcommandBuilder(), container)
      : subcommand
    : undefined;
...
export const analizeSubCommandParsed = (
  ...
  container.logger.debug(
    `[Subcommands-Plugin]: Analyzing subcommand "${subcommandParsed?.name ?? piece.name}" for parent "${parentCommandName}". Command has chatInputRun: ${!!piece.chatInputRun}`
  );

  if (!subcommandsRegistry) {
    subCommandsRegistry.set(
      parentCommandName,
      new Collection<string, SubcommandMappingCollection>().set(
        subcommandParsed?.name ?? piece.name,
        {
  ...
  const commandsCompare =
    subcommandParsed &&
    command.slashCommand &&
    isCommandOptionsUpdated(command.slashCommand, subcommandParsed);
  if (commandsCompare) {
  ...
  const parentCommand = container.stores.get('commands').get(parentCommandName) as
    | Subcommand
    | undefined;
  if (parentCommand) {
...
export const analizeSubcommandGroupParsed = (
  ...
  const commandsGroupCompare =
    subcommandParsed &&
    commandGroup.slashCommand &&
    isCommandOptionsUpdated(commandGroup.slashCommand, subcommandParsed);
  ...
  if (parentCommand) {
    ...
    if (subcommandGroup) {
      const subcommand = subcommandGroup.entries.find(
        (s) => s.name === subcommandName && s.type === 'method'
      );
      if (subcommand) {
        if (piece.chatInputRun) subcommand.chatInputRun = (i, c) => piece.chatInputRun?.(i, c);
