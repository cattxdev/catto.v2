...
export class EvidenceRoute extends Route {
  ...
  public async run(request: Route.Request, response: Route.Response) {
    ...
    if (request.method === 'GET') return this.handleGet(request, response, guildId);
    if (request.method === 'POST') return this.handlePost(request, response, guildId);
   ...
   */
  private async handleGet(request: Route.Request, response: Route.Response, guildId: string) {
    try {
   ...
   */
  private async handlePost(request: Route.Request, response: Route.Response, guildId: string) {
    try {
      const gate = await ApiGate.fromRequest(request, guildId);
      if (!gate)
        return response.status(401).json({ error: 'Unauthorized', code: 'NOT_AUTHENTICATED' });
  ...
  private async handleInitiate(
    ...
    if (!caseNumber || !filename || !mimeType || !sizeBytes) {
      return response
        .status(400)
        .json({ error: 'caseNumber, filename, mimeType, and sizeBytes are required' });
    }
  ...
  private async handleConfirm(
    ...
    const { evidenceId, contentHash } = body as { evidenceId: string; contentHash: string };

    if (!evidenceId || !contentHash) {
      return response.status(400).json({ error: 'evidenceId and contentHash are required' });
    }

    const evidence = await evidenceService.confirmUpload(evidenceId, contentHash);
  ...
  private async handleUrl(
    ...
    if (!caseNumber || !url) {
      return response.status(400).json({ error: 'caseNumber and url are required' });
    }
  ...
  private async handleBulkAmend(
    ...
    for (const evidenceId of evidenceIds) {
      try {
        ...
        errors.push({ evidenceId, error: err instanceof Error ? err.message : 'Unknown error' });
      }
    }
