	...
	public async run(oldEvent: GuildScheduledEvent | null, newEvent: GuildScheduledEvent) {
		...
		if (oldEvent.scheduledStartAt?.getTime() !== newEvent.scheduledStartAt?.getTime()) {
			changes.push({
				...
				value: `**Before:** ${time(oldEvent.scheduledStartAt!, TimestampStyles.LongDateTime)}\n**After:** ${time(newEvent.scheduledStartAt!, TimestampStyles.LongDateTime)}`
			});
		}
		...
		fields: [{ name: 'Event', value: `${newEvent.name} (${newEvent.id})`, inline: true }, ...changes],
