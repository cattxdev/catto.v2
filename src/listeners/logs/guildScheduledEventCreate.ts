...
export class GuildScheduledEventCreateListener extends Listener<typeof Events.GuildScheduledEventCreate> {
	...
	public async run(event: GuildScheduledEvent) {
		...
		...(event.scheduledStartAt ? [{ name: 'Start', value: time(event.scheduledStartAt, TimestampStyles.LongDateTime), inline: true }] : []),
			...(event.scheduledEndAt ? [{ name: 'End', value: time(event.scheduledEndAt, TimestampStyles.LongDateTime), inline: true }] : []),
			...(event.description ? [{ name: 'Description', value: event.description.substring(0, 1024) }] : []),
			...(event.channelId ? [{ name: 'Channel', value: `<#${event.channelId}>`, inline: true }] : [])
			],
