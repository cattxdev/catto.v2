...
export class ChannelUpdateListener extends Listener<typeof Events.ChannelUpdate> {
	...
	public async run(oldChannel: DMChannel | GuildChannel, newChannel: DMChannel | GuildChannel) {
		...
		if ('topic' in oldChannel && 'topic' in newChannel && oldChannel.topic !== newChannel.topic) {
			changes.push({
				name: 'Topic',
				value: `**Before:** ${oldChannel.topic || '*No topic*'}\n**After:** ${newChannel.topic || '*No topic*'}`.substring(0, 1024)
			});
		}
		...
		await logAction({
			...
			fields: [{ name: 'Channel', value: `${newChannel} (${newChannel.id})`, inline: true }, ...changes],
